((Utils) => {
    const App = {
      htmlElements: {
        ...
        hidden_eye: `<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        baby_svg: `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      init: () => {
        ...
        App.htmlElements.clean.addEventListener("click", App.handlers.cleanF)
      },
      handlers: {
        pokemonFinderFormOnSubmit: async (e) => {
          ...
          try {
            ...
            App.htmlElements.pokemonFinderOutput.innerHTML = `<h1 class="err">${error}</h1>`;
          }
        },
        cleanF: ()=>{
      ...
      templates: {
        ...
        errorCard: () => `<h1 class="err">There was an error</h1>`,
        pokemonCard: ({ id, name, weight, height, sprites, abilities}) => {
          const abi = abilities.map(({ability, is_hidden}) =>
                    `<ul>
                      <li>${ability.name}
                          ${is_hidden? App.htmlElements.hidden_eye : ""}</li>
                    </ul>`,
          );
          ...
          const pokemonList = pokemon.map(
            ({ pokemon, is_hidden }) =>
              `<li><a target="_blank" href="${pokemon.url}">${pokemon.name}${
                is_hidden ? App.htmlElements.hidden_eye : ""
              }</a></li>`
          );
